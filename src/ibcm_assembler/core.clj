...
(def instruction-type #{:h :io :s :a :d})
...
(defn encode-instruction [instr]
  ...
  (cond
    ...
    (let [address (instr :address)
          opcode (instr :opcode)] (str (opmap opcode) address))))

(defn encode [instr] (.toUpperCase (encode-instruction (parse-instr instr))))

(defn pad [instr]
  (if (> (count instr) 5) instr (str instr \tab)))

(defn format-instruction [op locn comm]
  (str (encode op) \tab locn \tab (pad op) comm))
...
(defn assemble [x]
  (let [lines (clojure.string/split-lines (slurp x))
    ...
    (apply str (interpose "\n" (map format-instruction instrs locns comments)))))

(defn -main [& args]
  (spit "out.ibcm" (assemble (nth args 0))))
